...
import * as service from './follow-up-statuses.service.js';
import type { CreateFollowUpStatusInput, UpdateFollowUpStatusInput , TenantScopedQuery} from './follow-up-statuses.schema.js';
...
export class FollowUpStatusesController {
  ...
  create = async (request: FastifyRequest, reply: FastifyReply) => {
    if (request.auth.rank < RANKS.SUPER_ADMIN) throw new ForbiddenError('Super admin only');
    const body = request.body as CreateFollowUpStatusInput;
  ...
  update = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const body = request.body as UpdateFollowUpStatusInput;
