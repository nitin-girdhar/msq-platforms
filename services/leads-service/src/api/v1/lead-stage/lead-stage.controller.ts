...
import * as service from './lead-stage.service.js';
import type { CreateLeadStageInput, UpdateLeadStageInput , TenantScopedQuery} from './lead-stage.schema.js';
...
export class LeadStageController {
  ...
  create = async (request: FastifyRequest, reply: FastifyReply) => {
    if (request.auth.rank < RANKS.SUPER_ADMIN) throw new ForbiddenError('Super admin only');
    const body = request.body as CreateLeadStageInput;
  ...
  update = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const body = request.body as UpdateLeadStageInput;
