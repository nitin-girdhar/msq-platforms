...
| GET/POST/PATCH | `/lookups/{lead-stage,lead-stage-outcome,interaction-types,follow-up-statuses,lead-sources,marketing-platforms,campaign-statuses}` (super_admin, `?tenant_id=`) | leads-service (N-6) |
| GET/POST/PATCH | `/lookups/{leave-types,employment-types,attendance-statuses}` (super_admin, `?tenant_id=`) | hr-service (N-6) |
| GET/POST/PATCH | `/lookups/{task-statuses,task-priorities}` (super_admin, `?tenant_id=`) | tasks-service (N-6) |
...
- **Shrunk token (P1.3)**: the JWT carries identity (`sub`, `email`), the coarse `platform_role` (`super_admin` | `tenant_admin` | `org_admin` | `member`), `org_id`/`tenant_id`, `licensed_products`, and `pwd_iat`/`jti` — but **no** global product role/rank. Role authority is resolved per request from the single iam ladder via `iam.fn_user_org_role`, so a stolen or stale token can never assert a rank it wasn't granted. `platform_role` drives which Postgres role `withRoleTx` selects (RLS) and platform-level gates; `licensed_products` is a UX convenience (the gateway's DB-backed entitlement gate remains authoritative).
...
P1.3 briefly gave each product its own rank scale, comparable only within that
product. **Tier C reversed that:** there is now ONE ladder, stored in
`iam.user_roles.rank` and resolved **from the DB per request** by
`iam.fn_user_org_role` (see JWT & auth above), never a header. Three scales that
could disagree were the direct cause of the "page renders but every call 403s"
class of bug; the per-product `<product>.roles`/`<product>.member_roles` tables
have since been dropped (see DB_model.md).

**Platform tiers** (`@platform/authz` `RANKS`; from `platform_role`): `member` 0 · `org_admin` 80 · `tenant_admin` 90 · `super_admin` 100.

**Unified ladder** (`@platform/rbac` `ANCHOR_RANK`; from `iam.user_roles.rank`): only the four
anchor roles have fixed ranks — `read_only` 0 · `org_admin` 80 · `tenant_admin` 90 ·
`super_admin` 100. Every other role is **tenant data**, tied to a department, with a rank an
admin picks in the open band between; never hard-code those, read them from the DB. The
historical per-product ladders (LMS `sales_representative` 20 … `lms_admin` 80; HR `hr_viewer`
0 … `hr_admin` 80; Tasks `task_member` 20 … `task_admin` 80) now live as ordinary tenant rows
on this one ladder.
