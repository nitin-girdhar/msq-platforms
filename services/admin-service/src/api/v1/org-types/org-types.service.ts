...
import * as repo from './org-types.repository.js';
...
export async function create(data: CreateOrgTypeInput) {
  try {
    ...
    if (msg.includes('unique')) throw new ConflictError('An org type with this name already exists.');
    throw err;
  }
}

export async function update(id: string, data: UpdateOrgTypeInput) {
  ...
  try {
    ...
    if (!row) throw new NotFoundError('Org type not found');
    return toApiRow(row);
  } catch (err) {
