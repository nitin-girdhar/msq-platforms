...
### P0.1 — Split the permissions package _(Opus)_
...
### P0.2 — Migrate imports off the shim _(Haiku)_
...
### P0.3 — Dependency-cruiser wall _(Sonnet)_
...
### P0.4 — Gateway entitlement enforcement _(Opus)_
...
### P1.1 — Design per-product role model _(Opus)_
...
### P1.2 — Implement role tables + grants + backfill _(Sonnet)_
...
### P1.3 — Shrink JWT + per-service role resolution _(Opus)_

> Change identity-service to issue the shrunk JWT (P1.1); update gateway header injection; make each product service resolve the user's _product_ role from its own `member_roles` table instead of trusting a global rank header. Keep login UX unchanged. Update `@platform/authz`, `@lms/authz`, `@hr/authz` call sites and Bruno tests.
...
### P2.1 — `hr.reporting_lines` model + repoint approvals *(Opus)*X
...
### P2.2 — Tests + docs for hierarchy split _(Sonnet)_
...
### P3.1 — Convert global lookups to tenant-scoped _(Sonnet)_
...
### P3.2 — Tenant default seeding _(Opus)_
...
### P3.3 — Tenant-context lookup-admin _(Sonnet)_
...
### P4.1 — Extract `@platform/ui-kit` _(Opus)_
...
### P4.2 — Product route-groups + feature packages _(Sonnet)_
...
### P4.3 — Split into per-product Next apps/images _(Opus)_
...
### P5.1 — Parent workspace + repo extraction plan _(Opus)_
...
### P5.2 — Self-contained Dockerfiles via `pnpm deploy` _(Sonnet)_
...
### P5.3 — Execute the extraction _(Sonnet)_
...
### P6.1 — Scaffold `gym-repo` on the clean template _(Opus)_
