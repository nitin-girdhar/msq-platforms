...
--
-- Product tables: hr, task, the tenant-catalog engine tables, and
...
-- Per-product role catalogs (lms/hr/task.roles) and their (user, product,
-- role) grant tables (lms/hr/task.member_roles) — P1.1's per-product RBAC —
-- were removed here (schema_version 1.19.0). Role/rank resolution runs on the
-- single iam.user_roles ladder now (iam.fn_user_org_role, resolveGlobalRole);
-- see docs/DB_model.md for the retired shape and db_scripts/one_time for the
-- one-off drop script used against servers that already had these tables.
