...
import * as repo from './hr-roles.repository.js';
import type { CreateHrRoleInput, UpdateHrRoleInput } from './hr-roles.schema.js';

export async function list(ctx: RoleTxContext) {
  return toApiRows(await repo.list(ctx));
}

export async function create(ctx: RoleTxContext, data: CreateHrRoleInput) {
  try {
    ...
    if (msg.includes('unique')) throw new ConflictError('An HR role with this name already exists.');
    throw err;
  }
}

export async function update(ctx: RoleTxContext, id: string, data: UpdateHrRoleInput) {
  ...
  try {
    ...
    if (!row) throw new NotFoundError('HR role not found');
    return toApiRow(row);
  } catch (err) {
