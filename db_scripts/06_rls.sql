...
--
-- These 5 tables are already tenant-scoped (tenant_id NOT NULL, script
-- 22/17). The 7 still-global LMS lookups (lead_stage etc.) are Half B.
-- (Originally 8 tables — lms.roles / hr.roles / task.roles were dropped when
-- Tier C consolidated role management onto the single iam ladder.)
--
...
-- ── hr.leave_types / employment_types / attendance_statuses  (hr_svc) ──
...
INSERT INTO public.schema_versions (version, description) VALUES
  ('1.18.0', 'N-6 Half A: tenant-scoped admin write RLS + product-role write GRANTs on the 5 tenant-scoped lookup tables (hr.leave_types/employment_types/attendance_statuses; task.task_statuses/task_priorities) so product services own super_admin lookup writes without BYPASSRLS')
