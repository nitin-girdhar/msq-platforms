...
import * as repo from './campaign-statuses.repository.js';
...
export async function create(ctx: RoleTxContext, data: CreateCampaignStatusInput) {
  try {
    ...
    if (msg.includes('unique')) throw new ConflictError('A campaign status with this name already exists.');
    throw err;
  }
}

export async function update(ctx: RoleTxContext, id: string, data: UpdateCampaignStatusInput) {
  ...
  try {
    ...
    if (!row) throw new NotFoundError('Campaign status not found');
    return toApiRow(row);
  } catch (err) {
