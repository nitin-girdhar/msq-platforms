...
export default function CreateApiClientModal({ open, onClose, orgs, isOrgAdmin, actorOrgId }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiClientsApi.create(body);
      setSuccess({ name: res.data.name, apiKey: res.data.api_key });
    } catch (err: unknown) {
  ...
  return (
    <Modal open={open} onClose={handleClose} title={success ? 'Token created' : 'New API token'} locked={pending}>
      {success ? (
        <div className="space-y-4">
          <p className="text-sm text-[#0F172A]">
            <span className="font-semibold">{success.name}</span> is ready to use.
          </p>
          <ApiKeyRevealPanel apiKey={success.apiKey} name={success.name} />
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="ac-name" className="text-xs font-semibold text-[#0F172A]">Name *</label>
            <input
              id="ac-name"
              value={name}
              onChange={(e) => setName(e.target.value)}
              disabled={pending}
              placeholder="e.g. Website lead intake"
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="ac-expires" className="text-xs font-semibold text-[#0F172A]">Expires on</label>
            <input
              id="ac-expires"
              type="date"
              value={expiresAt}
              ...
              className="rounded-xl border border-[#E2E8F0] bg-white px-3 py-2.5 text-sm text-[#0F172A] shadow-sm focus:border-[#0b6cbf] focus:outline-none focus:ring-2 focus:ring-[#0b6cbf]/20 disabled:cursor-not-allowed disabled:bg-[#F8FAFC]"
            />
            <span className="text-[11px] text-[#94A3B8]">Leave blank for a token that never expires.</span>
          </div>
