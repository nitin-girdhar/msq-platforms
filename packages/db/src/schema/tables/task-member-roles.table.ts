...
import { taskRolesTable } from './task-roles.table';

// (user, Task, role) grant — org-grained, tenant-isolated via RLS.
// tenant_id is derived from org_id by a DB trigger; never write it directly.
// See db_scripts/17 and docs/DB_model.md → task.member_roles.
export const taskMemberRolesTable = taskSchema.table('member_roles', {
  userId:    uuid('user_id').notNull().references(() => usersTable.id, { onDelete: 'cascade' }),
  orgId:     uuid('org_id').notNull().references(() => organizationsTable.id, { onDelete: 'cascade' }),
  tenantId:  uuid('tenant_id').notNull().references(() => tenantsTable.id, { onDelete: 'cascade' }),
  roleId:    uuid('role_id').notNull().references(() => taskRolesTable.id, { onDelete: 'restrict' }),
