...
- [x] **`@crm/*` → `@platform/*` rename DONE** — `db`, `types`, `service-auth`, `auth-constants`, `audit-log` renamed (`ui`→`@platform/ui-kit` and `permissions`→`@platform/authz` were already done). Bulk-replaced across 261 files (imports + `package.json` deps); stale `packages/validation` (dist/node_modules leftovers, no source — the real split into `@platform/validation`/`@lms/validation`/`@hr/validation`/`@task/validation` had already landed) deleted. Verified: `pnpm install`, `pnpm depcruise` (0 violations), `pnpm typecheck` (48/48 tasks) all green.
- [x] **DB migrations 15–20 — resolved differently than originally planned.** Investigation found only **15** (`entity.tenant_modules` key rename) and **16** (`ALTER SCHEMA crm RENAME TO lms` + `ALTER ROLE crm_service RENAME TO root_service`) are genuine "old shape" migrations — `01_init-db.sql`/`10_init-hr-task-schemas.sql` already create `lms`/`root_service`/the `'lms'` entitlement key directly, so both are guarded no-ops on a fresh install. They moved to `db_scripts/_migrations/` as the upgrade path for a pre-P1.0 deployment (see `_migrations/README.md`). **17–20 are purely additive/idempotent** (role tables, backfill, per-product grants, resolver functions) and run correctly against a brand-new database, so they were **not** archived — they stay in the normal numbered sequence. `db_deploy.ps1` (previously stale — only ran `01`–`05`, silently skipping `01_init-lookup-data.sql` and everything from `10` onward) now runs the full fresh-install sequence in order, with demo seed data toggleable via `-IncludeDemoSeed`. Not run against a live DB in this pass (files-only review, no DB available to verify against) — flagged as a follow-up to smoke-test.
- [ ] Carve `db_scripts/` (§3), `api-testing/` Bruno, `docker-compose*`, `tsconfig`/`turbo`, `.env.example` per repo; each product README states "requires shared-repo DB schemas first — cannot bootstrap alone." **Deferred to Stage D** — carving per-repo content only makes sense once the target repos exist (`git filter-repo` keeps whole files per §2, then a follow-up commit trims them — see §2's "Then:" notes per repo).
- [x] **`git filter-repo` installed** (`pip install git-filter-repo`, v2.47.0 — invoked as `git filter-repo`, not on PATH directly under `Scripts/`). Repo **not yet tagged** `pre-extraction` — do that as the last step before Stage D actually runs, not speculatively ahead of it.
