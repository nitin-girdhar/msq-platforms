...
import * as service from './employment-types.service.js';
import type {
...
export class EmploymentTypesController {
  ...
  create = async (request: FastifyRequest, reply: FastifyReply) => {
    if (request.auth.rank < RANKS.SUPER_ADMIN) throw new ForbiddenError('Super admin only');
    const data = await service.create(tenantCtx(request), request.body as CreateEmploymentTypeInput);
  ...
  update = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const data = await service.update(tenantCtx(request), id, request.body as UpdateEmploymentTypeInput);
    return reply.send({ success: true, data });
  };
}
