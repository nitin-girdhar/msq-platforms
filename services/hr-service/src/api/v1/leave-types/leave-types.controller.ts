...
export class LeaveTypesController {
  ...
  create = async (request: FastifyRequest, reply: FastifyReply) => {
    if (request.auth.rank < RANKS.SUPER_ADMIN) throw new ForbiddenError('Super admin only');
    const data = await service.create(tenantCtx(request), request.body as CreateLeaveTypeInput);
  ...
  update = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const data = await service.update(tenantCtx(request), id, request.body as UpdateLeaveTypeInput);
    return reply.send({ success: true, data });
  };
}
