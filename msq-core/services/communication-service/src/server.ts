...
app.get('/health', async () => ({ status: 'ok', service: 'communication-service' }));
...
const start = async () => {
  try {
    ...
    assertInternalServiceSecret({ nodeEnv: config.nodeEnv, logPrefix: '[communication-service] ' });
    await app.listen({ port: config.port, host: '0.0.0.0' });
    app.log.info(`Email configured: ${config.isEmailConfigured}`);
    app.log.info(`WhatsApp configured: ${config.isWhatsAppConfigured}`);
  } catch (err) {
    app.log.error(err);
    process.exit(1);
  }
