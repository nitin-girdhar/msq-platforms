...
export async function create(ctx: RoleTxContext, data: CreateInteractionTypeInput) {
  try {
    ...
    if (msg.includes('unique')) throw new ConflictError('An interaction type with this name already exists.');
    throw err;
  }
}

export async function update(ctx: RoleTxContext, id: string, data: UpdateInteractionTypeInput) {
  ...
  try {
    ...
    if (!row) throw new NotFoundError('Interaction type not found');
    return toApiRow(row);
  } catch (err) {
